...
const getColor = (value: number): string => {
  const color = CELL_COLORS.get(value.toString());
  return color ?? DEFAULT_CELL_COLOR;
};

const calculateSizes = (gameState: Array2D, width: number, height: number) => {
...
const drawCell = (
  ...
  if (cell.value) {
    ...
    ctx.fillText(
      cell.value.toString(),
      cell.x + (cell.maxWidth - textWidth) / 2,
      cell.y + cell.maxHeight / 1.7
    );
  }
};
